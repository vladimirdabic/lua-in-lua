...
m.evals = {
    ...
    ['class'] = function(self, node, environment)
        local class_table = {}

        -- create static fields and functions
        for _, class_field in ipairs(node.static_body) do
            class_table[class_field.name] = self:evaluate(class_field.value, environment)
        end
